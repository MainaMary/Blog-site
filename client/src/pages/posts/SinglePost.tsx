  ...
  return (
    <>
     ...
     <div className="block md:flex mt-12 gap-8">
        ...
        <p className="font-semibold text-dark-blue capitalize">{post.title}</p>
        <p className="capitalize">{post.body}</p>
        <p>{`${comments?.length} comments`}</p>
        </div>
       
     
      <div className="w-full md:w-[55%] ">{comments?.map(label=>
      <div className="px-4 py-2 border-solid border-2 border-gray-400 rounded-md my-2 " key={label.id}>
        <p>{label?.body}</p>
